...
## Instalacion facil (sin saber programar)

Si no usas GitHub ni programacion, puedes instalar el bot asi:

1. Descarga este proyecto como ZIP desde GitHub (boton **Code** → **Download ZIP**).
2. Descomprime la carpeta y abrela en tu computadora.
3. Instala Node.js desde <https://nodejs.org> (elige la version LTS).
4. Instala FFmpeg:
   - Windows: <https://www.gyan.dev/ffmpeg/builds/> (descarga *release full* y agrega la carpeta `bin` al PATH).
   - macOS: con Homebrew `brew install ffmpeg`.
   - Linux: usa tu gestor de paquetes, por ejemplo `sudo apt install ffmpeg`.
5. Dentro de la carpeta del bot, abre una terminal:
   - Windows: Shift + clic derecho → **Abrir ventana de PowerShell aqui**.
   - macOS/Linux: clic derecho → **Abrir en Terminal**.
6. Ejecuta este comando:

```bash
npm install
```

7. Crea un archivo llamado `.env` y copia el contenido de `.env.example`, luego completa tus datos.
8. Inicia el bot con:
